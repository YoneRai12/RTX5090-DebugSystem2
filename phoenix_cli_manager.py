...
class DiscordBot:
    ...
    def __init__(self, cfg: Config, logger: Logger, notifier: DiscordNotifier) -> None:
        self.cfg = cfg
        self.logger = logger
...
class PatchApplier:
    ...
    def apply_patch_set(self, patches: List[Dict[str, Any]]) -> Tuple[bool, str]:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # We want to validate Python syntax on the shadow copy BEFORE committing
            for real_path, temp_path in temp_map.items():
                # If it's a Python file, compile the shadow copy
                if real_path.suffix == ".py":
                    try:
                        import py_compile
                        py_compile.compile(str(temp_path), doraise=True)
                    except Exception as e:
                        # Do not commit if syntax fails
                        return False, f"Syntax error in patched {real_path.name}: {e}"

            # 4. Commit to real files (Atomic Move)
            ...
            try:
                for real_path, temp_path in temp_map.items():
                    ...
                    # Move temp file into place atomically. After this, temp_path is gone.
                    os.replace(str(temp_path), str(real_path))
            except Exception as e:
                self.logger.log("commit_failed", error=str(e))
                # Rollback: restore backups if commit fails
            ...
            # If we reach here, commit and optional tests succeeded
            return True, ""
...
def main() -> int:
    ...
    bot = DiscordBot(cfg, logger, handler.discord)
